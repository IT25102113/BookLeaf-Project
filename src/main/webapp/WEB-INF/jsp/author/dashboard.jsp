...
<div class="space-y-8 animate__animated animate__fadeIn">
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-accent/20 p-6">
        ...
        <form action="/author/books/add" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label class="block text-xs font-bold text-primary mb-1">Price ($) *</label><input type="number" name="price" step="0.01" min="0" required class="w-full px-4 py-2.5 bg-base border-2 border-accent/30 rounded-xl text-sm focus:outline-none focus:border-primary text-primary" placeholder="29.99"></div>
                        ...
                        <tbody>
                            <c:forEach var="book" items="${books}">
                                <tr class="border-b border-accent/10 hover:bg-base/50 transition-colors">
                                    ...
                                    <td class="py-3 px-4 font-bold text-primary">$${String.format("%.2f", book.price)}</td>
                                ...
                                <div id="editBookModal${book.id}" class="hidden fixed inset-0 bg-black/50 z-50 items-center justify-center p-4">
                                    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-8 animate__animated animate__fadeIn">
                                        ...
                                        <form action="/author/books/update" method="POST" class="space-y-4">
                                            ...
                                            <div class="grid grid-cols-2 gap-3">
                                                <div><label class="block text-xs font-bold text-primary mb-1">Price ($)</label><input type="number" name="price" step="0.01" value="${book.price}" required class="w-full px-4 py-2.5 bg-base border-2 border-accent/30 rounded-xl text-sm focus:outline-none focus:border-primary text-primary"></div>
