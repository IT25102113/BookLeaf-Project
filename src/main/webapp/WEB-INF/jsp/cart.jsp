        ...
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-4">
                <c:forEach var="item" items="${cart}" varStatus="s">
                    <div class="bg-white rounded-2xl shadow-lg border border-accent/20 p-5 flex items-center gap-5">
                        ...
                        <div class="text-right w-24"><p class="font-bold text-primary">$${String.format("%.2f", item.subtotal)}</p><p class="text-[10px] text-secondary">$${String.format("%.2f", item.unitPrice)} each</p></div>
            ...
            <div class="bg-white rounded-2xl shadow-xl border border-accent/20 p-6 h-fit sticky top-28">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between text-secondary"><span>Items (${cart.size()})</span><span>$${String.format("%.2f", cartTotal)}</span></div>
                    ...
                    <div class="flex justify-between text-primary font-bold text-lg"><span>Total</span><span>$${String.format("%.2f", cartTotal)}</span></div>
                </div>
