        ...
        <div class="space-y-3">
            <c:forEach var="item" items="${cart}">
                <div class="flex justify-between items-center text-sm py-2 border-b border-accent/10">
                    ...
                    <span class="font-bold text-primary">$${String.format("%.2f", item.subtotal)}</span>
                </div>
            </c:forEach>

            <div class="flex justify-between text-primary text-sm pt-2"><span>Subtotal</span><span class="font-bold">$${String.format("%.2f", cartTotal)}</span></div>
                ...
                <div class="flex justify-between items-center text-sm py-2 bg-green-50 px-3 rounded-xl border border-green-200">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <span class="text-xs text-green-600 ml-1">(-${String.format("%.0f", discount.percentage)}%)</span>
                        </div>
                    </div>
                    <span class="font-bold text-green-600">-$${String.format("%.2f", discountAmount)}</span>
                </div>
            </c:if>
            ...
            <div class="flex justify-between text-primary font-bold text-lg pt-2 border-t-2 border-accent/20">
                ...
                <div class="text-right">
                    <c:if test="${not empty discount}">
                        <span class="text-sm text-secondary line-through mr-2">$${String.format("%.2f", cartTotal)}</span>
                    </c:if>
                    <span class="text-primary">$${String.format("%.2f", discountedTotal)}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div id="panelCard">
            <form action="/checkout/process" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div><label class="block text-sm font-bold text-primary mb-1.5">Cardholder Name</label><input type="text" placeholder="John Doe" class="w-full px-4 py-3 bg-base border-2 border-accent/30 rounded-xl text-sm focus:outline-none focus:border-primary"></div>
                    ...
                    <button type="submit" class="w-full py-3.5 bg-primary text-white font-bold rounded-xl hover:bg-dark transition-all shadow-lg text-sm flex items-center justify-center gap-2"><i class="fas fa-lock"></i> Pay $${String.format("%.2f", discountedTotal)}</button>
                </div>
            </form>
        </div>
